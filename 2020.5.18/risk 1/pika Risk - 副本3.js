...
let pika = {
  ...
  eat: function () {
    if (this.isLiving()) {
      if (this.health < 100) {
        ...
        view.displayPikaSay('皮卡丘补了10滴血');
        document.blood.src = "../image1/riskpikaq加血.png";
      }else{
        view.displayPikaSay('皮卡丘现在是满血状态');
      };
    };
  },
  ...
  attack: function() {
    if(this.health > 0) {
      ...
      view.displayPikaSay( '皮卡丘对浊然发动了攻击');
      this.isLiving();
    }else{
      view.displayPikaSay('皮卡丘死了！');
      setTimeout(function(){
 ...
 defense: function() {
   if(enemy.aida()){
     ...
     view.displayPikaSay('皮卡丘试图抵挡攻击！');
   }else{
     view.displayPikaSay('白毛还没有攻击皮卡丘');
   }
 }
};
...
let enemy = {
  ...
  dead: function () {
    clearInterval(this.liveInterval);
    this.isLive = false;
    view.displayPikaSay('浊然：吾命由我，不，不由....');
    ...
    if(this.health > 0){
      ...
      this.liveInterval = setInterval(function(){
        if(time.isLiving) {
          ...
          view.displayPikaSay('浊然对皮卡丘进行投掷攻击');
          times.progress();
          time.checkStatus();
        };
      }, 4000);
    }else{
      view.displayPikaSay('恭喜你成功击败了浊然！');
      setTimeout(function(){
        location.href='./pikaqRisk.html';
      }, 2000);
    };
